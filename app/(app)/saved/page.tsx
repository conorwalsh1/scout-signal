      ...
      {plan !== "pro" && savedLimitReached && (
        <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/30 p-4 flex items-center justify-between gap-4">
          <p className="text-sm text-amber-800 dark:text-amber-200">
            {plan === "free"
              ? "You’ve reached the Free limit of 10 saved companies."
              : "You’ve reached the Basic limit of 25 saved companies."}
          </p>
          <Link href="/pricing">
            <Button size="sm">{plan === "free" ? "View plans" : "Upgrade to Pro"}</Button>
          </Link>
        </div>
      )}
